...
class Player:
    def __init__(self,x,y, size = 20,invis = False):
        self.size = size
        self.invis = invis
        self.x = x
        self.y = y
...
class Stall(Base):
    def __init__(self,x,y,price,red = 150 , green= 75, blue = 0, size = 75):
...
class Shop:
    ...
    def generate(self):
        ...
        self.LivesUp = Stall(30, (Game_Screen_height //4),3)
        self.Invis = Stall(30, (Game_Screen_height //4) * 2,8)
        self.SpeedUp = Stall(30, (Game_Screen_height //4) * 3,2)
        self.Dash = Stall(Game_Screen_width - 110, (Game_Screen_height //4) * 2,5)
...
class Arena:
    ...
    def generate_walls(self):
        ...
        #    Wall1 = Wall(Game_Screen_width, Game_Screen_height)
        #    collision = False
        #    for wall in walls:
         #       if Wall1.colliderect(wall.rect):
         #           collision = True
        #            break
        #    if Wall1.colliderect(self.End.rect):
        #        collision = True
        #        break
        #    if Wall1.colliderect(self.Home.rect):
        #        collision = True
        #        break
        #    for hunter1 in self.huntersList:
        #        if Wall1.colliderect(hunter1.rect):
        #            collision = True
        #            break
        #    for bouncer in self.bouncersList:
        #        if Wall1.colliderect(bouncer.rect):
        #            collision = True
        #            break
        #    if not collision:
        #        walls.append(Wall1)
    ...
    def update(self):
        ...
        if lagtime == False:
            ...
            if keys[pygame.K_UP]:
                ...
                if keys[pygame.K_SPACE] and arena_current.player.dash and arena_current.player.dashCooldown <= 0:
                    arena_current.player.rect.y -= arena_current.player.speed*20
                    arena_current.player.Time_Used = pygame.time.get_ticks()
                    arena_current.player.dashCooldown = -pygame.time.get_ticks() + (self.player.Time_Used + self.player.dashDelay)
                else:
                    arena_current.player.rect.y -= arena_current.player.speed
            if keys[pygame.K_DOWN]:
                self.keyPress = True
            ...
            for wall in self.walls:
                if arena_current.player.colliderect(wall) and arena_current.player.invis == False:
...
while init == True:
    ...
    shop = Shop()
    running = True
    if score % 5 == 0 and score != 0 and passed == 1:
        shop.generate()
        shopIn = True
        while shopIn:
            for event in pygame.event.get():
